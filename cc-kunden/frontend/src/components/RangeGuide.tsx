...
const TEMP_FACTORS: [number, number, string, string][] = [
  ...
  [20,  1.00, 'text-cc-600',  'bg-cc-50'],
  [25,  0.97, 'text-cc-500',  'bg-cc-50'],
...
const DESTINATIONS = [
  { name: 'Köln Zentrum', km: 28 },
  { name: 'Bonn', km: 35 },
  { name: 'Siegburg', km: 18 },
  { name: 'Gummersbach', km: 22 },
  { name: 'Düsseldorf', km: 75 },
  { name: 'Koblenz', km: 95 },
]
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="space-y-6">
            {/* Vehicle selector */}
            <div>
              <label className="block text-sm font-semibold text-surface-700 mb-2">Fahrzeugmodell</label>
                  ...
                  <button
                    ...
                    className={`flex items-center gap-3 p-3 rounded-xl border-2 text-left transition-all ${
                      selectedVehicle === v.id
                        ? 'border-cc-500 bg-cc-50'
                        : 'border-surface-200 bg-white hover:border-cc-300'
                    }`}
                  >
                    <span className="text-xl">{v.icon}</span>
                    <div className="flex-1 min-w-0">
                      <p className="font-medium text-surface-900 text-sm truncate">{v.name}</p>
                      <p className="text-xs text-surface-500">{v.rangeMin}–{v.rangeMax} km WLTP · {v.seats} Sitze</p>
                    </div>
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                      selectedVehicle === v.id ? 'bg-cc-600 text-white' : 'bg-surface-100 text-surface-600'
                    }`}>
                      {v.subcategory}
                    </span>
                  </button>
            ...
            <div className="bg-surface-50 rounded-xl p-5">
              <div className="flex justify-between items-center mb-3">
                <label className="text-sm font-semibold text-surface-700">Außentemperatur</label>
                <span className={`text-lg font-bold font-display ${colorClass}`}>{temp > 0 ? '+' : ''}{temp}°C</span>
              </div>
              <input
          ...
          <div className="space-y-4">
            ...
            <div className={`rounded-2xl p-6 text-center ${bgClass} border-2 border-current`}
              ...
              <div className="text-6xl font-bold font-display mb-1" style={{ color: temp < 0 ? '#1d4ed8' : temp < 15 ? '#0e7490' : temp < 25 ? '#00a651' : '#d97706' }}>
                ~{adjustedRange}
              </div>
              <div className="text-lg font-semibold text-surface-700">km Reichweite</div>
            ...
            <div className="bg-surface-50 rounded-xl p-5">
              <h4 className="font-bold text-surface-900 text-sm mb-3">
                Beispielziele ab Overath
              </h4>
                    ...
                    <div key={d.name} className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <span className={`w-2.5 h-2.5 rounded-full flex-shrink-0 ${
                          status === 'green' ? 'bg-cc-500' :
                          status === 'yellow' ? 'bg-amber-400' : 'bg-red-400'
                        }`} />
                        <span className="text-sm text-surface-700">{d.name}</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <span className="text-xs text-surface-500">{d.km} km</span>
                        <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                          status === 'green' ? 'bg-cc-100 text-cc-700' :
            ...
            <div className="bg-white border border-surface-200 rounded-xl p-4">
              <h4 className="font-semibold text-surface-800 text-sm mb-3">Reichweite nach Temperatur</h4>
              <div className="grid grid-cols-2 gap-1">
                {TEMP_FACTORS.map(([t, f, tc]) => (
                  <div
                    key={t}
                    className={`flex justify-between px-3 py-1.5 rounded-lg text-xs ${
                      temp === t ? 'bg-cc-100 font-bold' : 'bg-surface-50'
                    }`}
                  >
                    <span className="text-surface-600">{t > 0 ? '+' : ''}{t}°C</span>
