  ...
  return (
    <div
      className={`relative cursor-pointer rounded-2xl border-2 transition-all duration-300 overflow-hidden ${
        active
          ? 'border-cc-500 shadow-warm-md bg-white'
      ...
      <div className="flex items-start gap-4 p-5">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <span className={active ? 'text-cc-600' : 'text-surface-400'}>{step.icon}</span>
  ...
  return (
    <div className="mb-6">
      ...
      <div className="space-y-2">
        {questions.map((q, i) => (
          <details key={i} className="group rounded-xl border border-surface-200 overflow-hidden bg-white">
            <summary
            ...
            >
              <span className="text-sm font-medium text-surface-800 pr-4">{q.question}</span>
              <ChevronDown className="w-4 h-4 text-surface-400 flex-shrink-0 group-open:rotate-180 transition-transform" />
            </summary>
