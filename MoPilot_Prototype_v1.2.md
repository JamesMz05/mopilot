...
**Prototyp v1.2**   Datum: 3. Mai 2026
...
20. Admin-Benutzerverwaltung & Einladungssystem – NEU in v1.1
    ...
    - 20.9 Commits
21. Quiz-Feature: CC Mobilitats-Check – NEU in v1.2
    - 21.1 Übersicht & Zweck
    - 21.2 Architektur
    - 21.3 Datenmodell
    - 21.4 API-Referenz
    - 21.5 Frontend-Komponenten
    - 21.6 Cookie & Tracking
    - 21.7 E-Mail-Notification
    - 21.8 Admin-Dashboard (Quiz-Stats)
    - 21.9 Quiz-Inhalt (14 Fragen)
...
MoPilot ist ein KI-gestützter Mobilitätsassistent für E-Carsharing im ländlichen Raum. Der Prototyp v1.2 erweitert v1.1 um das **CC Quiz-Feature (Mobilitats-Check)**: ein interaktives 14-Fragen-Quiz auf cc-kunden.mopilot.website/quiz mit anonymem Tracking, 30-Tage-Cookie-Lock, automatischer E-Mail-Benachrichtigung bei Abschluss, Gutscheincode-Ausgabe (ECOBONUS26) und einem Admin-Dashboard mit KPI-Kacheln, Trichter-Diagramm, Antwort-Heatmap und CSV-Export unter mopilot.website/admin/quiz-stats. Die Quiz-Logik läuft im main-backend (nicht im stateless cc-backend). Alle v1.1-Features bleiben erhalten: Admin-Einladungssystem, vereinheitlichte Infrastruktur, rollenspezifische Dashboards, KI-Chat, Landing Page, DB-Authentifizierung.
...
### Funktionsumfang v0.9 / v1.0 / v1.2

**Neu in v1.2 (CC Quiz Migration & Analytics)**
- CC Mobilitats-Check: 14-Fragen-Quiz auf cc-kunden.mopilot.website/quiz, Migration von sharing-community.de/quiz/
- Quiz-Logik im main-backend (mopilot-DB), cc-backend bleibt stateless
- 3 neue DB-Tabellen: quiz_definitions, quiz_sessions, quiz_events
- 10 neue API-Endpoints (6 Quiz + 4 Stats) im main-backend
- Cookie-basierter 30-Tage-Soft-Lock (Cookie-Domain `.mopilot.website`)
- Automatische E-Mail-Notification an register@sharing-community.de bei Quiz-Abschluss
- Gutscheincode ECOBONUS26 (20 Stunden Startguthaben), Ziel-URL cc.evemo.app/registration
- Admin-Dashboard unter /admin/quiz-stats: KPI-Kacheln, Trichter, Antwort-Heatmap, CSV-Export
- Rollengeschützt: Plattform-Support, Projektträger, Fahrzeugsteller, Validierungsstelle
- Embed-Variante unter /embed/quiz mit CSP frame-ancestors für sharing-community.de
- Verlinkung im Betreiber-Dashboard (/dashboard/betreiber)
- Multi-Quiz-fähige Architektur (ZEO-Variante vorbereitet, nicht implementiert)
...
**cc-kunden.mopilot.website (CC Kundenassistent) – NEU in v0.72, erweitert in v1.2**
...
- **NEU v1.2:** Mobilitats-Check Quiz (/quiz), Embed-Variante (/embed/quiz), Cross-Origin-Aufruf zum main-backend
...
|-----------|-------|---------|
| `mopilot` | Hauptsystem – Users, Stations, Vehicles, Tariffs, FAQs, ChatMessages, **Quiz (quiz_definitions, quiz_sessions, quiz_events)** | Hauptsystem-Backend (async), Ideen-Backend (sync, für User-Sync) |
...
Das Quiz-Feature ist ein interaktiver 14-Fragen-Mobilitats-Check auf cc-kunden.mopilot.website/quiz, migriert von sharing-community.de/quiz/. Ziel ist die Schliessung der Tracking-Lücke: Jeder erfolgreiche Quiz-Abschluss wird per E-Mail an register@sharing-community.de gemeldet, unabhängig davon ob der ausgegebene Gutscheincode eingelöst wird.

**Zielgruppen:**
- CC-Interessenten (Quiz-Teilnehmer)
- Stationspaten und Vorstand (Einbettung auf sharing-community.de)
- Plattform-Support und Strategie-Rollen (Dashboard-Analyse)

**Kernfunktionen:**
- 14 Fragen in 8 Kategorien mit Sofort-Feedback (richtig/falsch + Erklärung)
- Gutscheincode ECOBONUS26 (20 Stunden Startguthaben) bei Abschluss
- Cookie-basierter 30-Tage-Soft-Lock (Wiederkehr zeigt bestehenden Code)
- Automatische E-Mail-Notification mit anonymisierten Metriken
- Admin-Dashboard mit KPI-Kacheln, Frage-Trichter, Antwort-Heatmap, CSV-Export

### 21.2 Architektur

**Wichtig:** Die Quiz-Logik läuft im **main-backend** (hat die mopilot-DB-Anbindung), nicht im stateless cc-backend.

```
Browser (cc-kunden.mopilot.website/quiz)
    │
    ├── Quiz-UI (Next.js, cc-frontend)
    │       │
    │       ├── GET /api/quiz/cc-mobilitaets-check  ──────┐
    │       ├── POST /api/quiz/{slug}/start               │
    │       ├── POST /api/quiz/sessions/{id}/answer        ├── Cross-Origin
    │       ├── POST /api/quiz/sessions/{id}/complete      │   credentials: 'include'
    │       └── POST /api/quiz/sessions/{id}/abandon       │
    │                                                      ▼
    │                                          main-backend (api.mopilot.website)
    │                                              │
    │                                              ├── quiz.py (Router)
    │                                              ├── quiz_stats.py (Stats-Router)
    │                                              ├── models/quiz.py (SQLAlchemy)
    │                                              └── services/quiz_mail.py (SMTP)
    │                                                      │
    │                                                      ▼
    │                                              PostgreSQL (mopilot DB)
    │                                              ├── quiz_definitions
    │                                              ├── quiz_sessions
    │                                              └── quiz_events
    │
    └── Cookie: mopilot_quiz_session
        Domain: .mopilot.website
        Max-Age: 30 Tage, SameSite=Lax, Secure
```

**CORS-Konfiguration:**
- Origin: `https://cc-kunden.mopilot.website` ist whitelisted
- `allow_credentials=True` (Cookie-Übertragung)
- Build-Arg `NEXT_PUBLIC_QUIZ_API_BASE` im CC-Frontend-Dockerfile (Default: `https://api.mopilot.website`)

### 21.3 Datenmodell

Drei neue Tabellen in der `mopilot`-Datenbank:

**quiz_definitions**

| Spalte | Typ | Constraints | Beschreibung |
|--------|-----|-------------|-------------|
| id | UUID | PK, default uuid4 | Primärschlüssel |
| slug | VARCHAR(100) | UNIQUE, NOT NULL | URL-Slug (z.B. `cc-mobilitaets-check`) |
| title | VARCHAR(200) | NOT NULL | Anzeige-Titel |
| operator | VARCHAR(20) | NOT NULL | Betreiber-Kennung (`CC` / `ZEO`) |
| voucher_code | VARCHAR(50) | NOT NULL | Gutscheincode bei Abschluss |
...
| notification_email | VARCHAR(200) | NOT NULL | E-Mail-Empfänger für Benachrichtigungen |
...
| updated_at | TIMESTAMPTZ | NOT NULL, DEFAULT now() | Letztes Update (onupdate) |

**quiz_sessions**

| Spalte | Typ | Constraints | Beschreibung |
|--------|-----|-------------|-------------|
| id | UUID | PK (= Cookie-Wert) | Session-ID, explizit gesetzt |
| quiz_id | UUID | FK → quiz_definitions.id (CASCADE), NOT NULL | Zugehöriges Quiz |
| started_at | TIMESTAMPTZ | NOT NULL, DEFAULT now() | Start-Zeitpunkt |
| completed_at | TIMESTAMPTZ | nullable | Abschluss-Zeitpunkt |
| abandoned_at | TIMESTAMPTZ | nullable | Abbruch-Zeitpunkt |
| correct_count | INTEGER | NOT NULL, DEFAULT 0 | Anzahl richtiger Antworten |
| total_count | INTEGER | NOT NULL, DEFAULT 0 | Anzahl beantworteter Fragen |
| referrer | VARCHAR(255) | nullable | HTTP-Referrer |
| user_agent_class | VARCHAR(20) | nullable | `mobile` / `desktop` / `tablet` |
| notification_sent | BOOLEAN | NOT NULL, DEFAULT false | E-Mail erfolgreich versendet? |
...
| session_id | UUID | FK → quiz_sessions.id (CASCADE), NOT NULL, INDEX | Zugehörige Session |
| question_index | INTEGER | nullable | Frage-Index (0-basiert) |
| selected_index | INTEGER | nullable | Gewählte Antwort (0-basiert) |
...
**Beziehungen:**
- `quiz_sessions.quiz_id` → `quiz_definitions.id` (CASCADE)
- `quiz_events.session_id` → `quiz_sessions.id` (CASCADE)

### 21.4 API-Referenz

Alle Endpoints laufen auf dem **main-backend** (`https://api.mopilot.website`).

**Quiz-Endpoints (öffentlich, kein Auth):**

| Methode | Pfad | Beschreibung | Request-Body | Response | Status-Codes |
|---------|------|-------------|-------------|----------|-------------|
| GET | `/api/quiz/{slug}` | Quiz-Definition ohne `correct_index` | — | `QuizDefinitionPublic` | 200, 404 |
| POST | `/api/quiz/{slug}/start` | Session erzeugen, Cookie setzen | `{referrer?, user_agent_class?}` | `{session_id, quiz}` | 200, 404 |
| POST | `/api/quiz/sessions/{id}/answer` | Antwort speichern | `{question_index, selected_index}` | `{is_correct, correct_index, explanation, progress}` | 200, 400, 404 |
| POST | `/api/quiz/sessions/{id}/complete` | Quiz abschliessen, Code + E-Mail | — | `{score, total, voucher_code, voucher_label, registration_url}` | 200, 400, 404 |
| POST | `/api/quiz/sessions/{id}/abandon` | Beacon (sendBeacon) | — | `{ok: true}` | 200 |
| GET | `/api/quiz/sessions/{id}/state` | Session-Status prüfen | — | `{status, voucher_code?, score?, total?}` | 200 |

**Stats-Endpoints (Auth: Plattform-Support+):**

| Methode | Pfad | Beschreibung | Query-Parameter | Response |
|---------|------|-------------|----------------|----------|
| GET | `/api/quiz/stats/overview` | KPIs | `from`, `to`, `slug` | `{sessions_total, sessions_completed, completion_rate, avg_score, vouchers_issued, by_device, by_referrer}` |
| GET | `/api/quiz/stats/funnel` | Trichter pro Frage | `slug`, `from`, `to` | `{questions: [{index, reached, answered_correct, answered_wrong}]}` |
| GET | `/api/quiz/stats/answer-heatmap` | Antwortverteilung | `slug`, `from`, `to` | `{questions: [{index, question, correct_index, answers: [{index, text, count}]}]}` |
| GET | `/api/quiz/stats/export.csv` | CSV-Export | `slug`, `from`, `to` | CSV-Datei (`;`-Delimiter) |

**CORS-Hinweis:** Origin `https://cc-kunden.mopilot.website` mit `allow_credentials=True` für Cookie-Übertragung.

### 21.5 Frontend-Komponenten

**Quiz-Player (CC-Frontend):**

Dateien existieren in **beiden** CC-Frontend-Verzeichnissen (müssen synchron gehalten werden):
- `cc-kunden/frontend/src/components/quiz/` (Entwicklung)
- `MoPilot_CC_Kunden/frontend/src/components/quiz/` (Docker-Build)

| Komponente | Datei | Funktion |
|------------|-------|----------|
| QuizContainer | `QuizContainer.tsx` | Hauptcontainer, State-Management, API-Aufrufe |
| QuestionCard | `QuestionCard.tsx` | Einzelne Frage mit 4 Antwortoptionen |
| ProgressBar | `ProgressBar.tsx` | Fortschrittsanzeige (x/14) |
| MotivationButton | `MotivationButton.tsx` | Animierter Weiter-Button |
| ResultScreen | `ResultScreen.tsx` | Ergebnis mit Score, Gutscheincode, Registrierungs-Link |
| CookieConsentBanner | `CookieConsentBanner.tsx` | Einmaliger Cookie-Hinweis (Funktionscookie) |

**Quiz-API-Client:**
- `cc-kunden/frontend/src/lib/quiz-api.ts` (in beiden Verzeichnissen)

**Quiz-Routen (Next.js):**
- `/quiz` → Quiz-Player (öffentlich, ohne Login)
- `/embed/quiz` → iframe-fähige Variante (öffentlich)

**Admin-Dashboard (Hauptsystem-Frontend):**
- `frontend/app/admin/quiz-stats/page.tsx`
- Vier Bereiche: KPI-Kacheln, Trichter-Diagramm, Antwort-Heatmap, CSV-Export
- Filter: Zeitraum (von/bis), Gerät, Referrer
...
| Secure | true |
| HttpOnly | false |
| Wert | UUID der Quiz-Session |

**30-Tage-Soft-Lock:** Bei Wiederkehr innerhalb von 30 Tagen wird der bestehende Session-Status geprüft. Ist das Quiz abgeschlossen, wird direkt der Gutscheincode angezeigt – es wird kein neuer Code generiert.

**Was wird getrackt (anonymisiert):**
- Session-UUID (kein Personenbezug)
- Geräteklasse (mobile/desktop/tablet)
- Referrer (woher kam der Besucher)
- Score (Anzahl richtige Antworten)
...
**DSGVO-Hinweis:** Der Cookie ist ein Funktionscookie (kein Tracking-Cookie im Sinne der ePrivacy-Richtlinie). Der CookieConsentBanner informiert einmalig über den Einsatz.

### 21.7 E-Mail-Notification

Bei jedem erfolgreichen Quiz-Abschluss wird eine E-Mail an den in `quiz_definitions.notification_email` konfigurierten Empfänger gesendet.

**Konfiguration:**
- Empfänger: `register@sharing-community.de` (konfigurierbar in DB)
- SMTP: IONOS (`smtp.ionos.de:587`, STARTTLS)
...
| Start | `03.05.2026 14:22:15` |
| Ende | `03.05.2026 14:28:43` |
| Dauer | `388 Sekunden` |
| Score | `12 / 14` |
| Gerät | `mobile` |
| Referrer | `https://sharing-community.de` |
| Gutscheincode | `ECOBONUS26` |

**Servicedatei:** `backend/app/services/quiz_mail.py`

### 21.8 Admin-Dashboard (Quiz-Stats)

**URL:** `https://mopilot.website/admin/quiz-stats`

**Rollengeschützt:** Plattform-Support, Projektträger, Fahrzeugsteller, Validierungsstelle, MOPILOT_TEAM, Admin

**Vier Bereiche:**

1. **KPI-Kacheln:** Sessions gesamt, abgeschlossen, Abschlussrate, Durchschnittsscore, ausgegebene Gutscheine, Aufschlüsselung nach Gerät und Referrer
2. **Trichter-Diagramm:** Pro Frage: wie viele Teilnehmer haben sie erreicht, richtig/falsch beantwortet
3. **Antwort-Heatmap:** Verteilung der gewählten Antworten pro Frage (welche Fragen sind besonders schwer?)
4. **CSV-Export:** Download aller Sessions mit Session-ID, Datum, Score, Gerät, Referrer, Abschluss-Status, Dauer

**Filter:** Zeitraum (von/bis), Geräteklasse, Referrer

**Verlinkung:** Das Betreiber-Dashboard unter `/dashboard/betreiber` enthält einen Link "CC Carsharing Quiz mit Gutschein" zum Quiz-Stats-Dashboard.
...
Quelle: `quiz_definitions.questions_json` für Slug `cc-mobilitaets-check`. Bei Diskrepanzen ist die Datenbank die maßgebliche Quelle.

**Bearbeitung:** Aktuell nur per DB-Update (Admin-UI für Inhalts-Pflege ist Folge-Phase).

### 21.10 Operative Hinweise

- **Build-Arg `NEXT_PUBLIC_QUIZ_API_BASE`:** Muss im CC-Frontend-Dockerfile durchgereicht werden (Default: `https://api.mopilot.website`)
- **Middleware-Ausnahme:** `/quiz` und `/embed` sind ohne Login erreichbar (Ausnahme in Next.js Middleware)
- **Embed-CSP:** `frame-ancestors` im next.config.js erlaubt `https://sharing-community.de`
- **Zwei CC-Frontend-Verzeichnisse:** Änderungen IMMER in `cc-kunden/frontend/` UND `MoPilot_CC_Kunden/frontend/` pflegen
- **Uptime-Kuma-Monitore:** Für Quiz-Endpoints noch ausstehend

**Dateien im Backend:**

| Datei | Funktion |
|-------|----------|
| `backend/app/api/quiz.py` | Quiz-Endpoints (6 Routen) |
| `backend/app/api/quiz_stats.py` | Stats-Endpoints (4 Routen) |
...
**Dateien im CC-Frontend:**

| Datei | Funktion |
|-------|----------|
| `src/components/quiz/QuizContainer.tsx` | Hauptcontainer |
| `src/components/quiz/QuestionCard.tsx` | Frage-Darstellung |
| `src/components/quiz/ProgressBar.tsx` | Fortschrittsbalken |
| `src/components/quiz/MotivationButton.tsx` | Weiter-Button |
| `src/components/quiz/ResultScreen.tsx` | Ergebnis-Screen |
| `src/components/quiz/CookieConsentBanner.tsx` | Cookie-Banner |
| `src/lib/quiz-api.ts` | API-Client |
| `src/app/quiz/page.tsx` | Quiz-Route |
| `src/app/embed/quiz/page.tsx` | Embed-Route |

**Dateien im Hauptsystem-Frontend:**
...
- **Anwenderhandbuch:** `HANDBUCH_QUIZ_ANWENDER.md` — Fur Vorstand, Stationspaten, Marketing
- **IT-Handbuch:** `HANDBUCH_QUIZ_IT.md` — Fur IT-Verantwortliche und Entwickler

### 21.12 Bekannte Einschränkungen / Folge-Phasen

- **Statischer Gutscheincode:** ECOBONUS26 für alle Teilnehmer – individuelle Codes als Folge-Phase möglich
- **ZEO-Variante:** Multi-Quiz-Architektur vorbereitet, ZEO-Inhalte noch zu erarbeiten
- **Frage-Bearbeitung:** Aktuell nur per DB-Update – Admin-UI für Inhalts-Pflege als Folge-Phase
- **A/B-Testing:** Nicht enthalten
- **Uptime-Kuma:** Monitore für Quiz-Endpoints noch einzurichten

### 21.12 Commits

| Hash | Beschreibung |
|------|-------------|
| `2e8774d` | feat(quiz): CC Mobilitats-Check — Quiz, Tracking, Dashboard |
| `2624b80` | feat(dashboard): Link zu Quiz-Stats im Betreiber-Dashboard |
| `158dcfd` | Merge branch 'feat/cc-quiz' — CC Quiz Migration & Analytics |
